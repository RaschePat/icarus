...
import { getStudentCourses, getUnits, getSections } from "@/lib/api";
...
export default function StudentPage() {
  ...
  const handleSectionClick = (section: Section) => {
    router.push(`/student/lesson/${section.lesson_id}`);
  };
  ...
  return (
    <div className="max-w-screen-xl mx-auto px-6 py-6 flex flex-col gap-6">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-0.5">배정받은 과정 → 단원 → 섹션을 선택해 수업을 시작하세요.</p>
      </div>
      ...
      {!loading && courses.length === 0 && (
        <div className="card text-center py-10">
          ...
          <p className="text-slate-600 text-xs mt-1">관리자에게 과정 배정을 요청하세요.</p>
        </div>
      )}
