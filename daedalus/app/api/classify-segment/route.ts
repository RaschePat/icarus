...
function buildSystem(topic?: string, keywords?: string[], libraries?: string[]): string {
  ...
  if (libraries && libraries.length > 0) {
    contextLines.push(`사용 라이브러리/API: ${libraries.join(", ")}`);
  }
  const contextBlock =
...
export async function POST(req: NextRequest) {
  const { transcript, topic, keywords, libraries } = (await req.json()) as {
    transcript?: string;
    topic?:      string;
    keywords?:   string[];
    libraries?:  string[];
  ...
  try {
    ...
    const res = await client.messages.create({
      ...
      max_tokens: 1024,
      system:     buildSystem(topic, keywords, libraries),
